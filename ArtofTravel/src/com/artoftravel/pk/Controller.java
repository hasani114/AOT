...
//		response.getWriter().append("Served at Main Controller: ").append(request.getContextPath());
		...
		//Get Values from Request from User
		String[] allAct = request.getParameterValues("Activities");
		
		//Convert Array into String
		String activities = "";
		for (int i=0;i<allAct.length;i++) {
			System.out.println(allAct[i]);
			activities = activities + allAct[i] + ",";
		}
		...
		int Last = allAct.length;
		Query = Query + "%"+allAct[Last-1]+"%;";
